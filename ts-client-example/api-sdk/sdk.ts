...
export default function BuildSDK(apiBaseUrl: string) {
  return {
    users: {
      create: (
        args: {} & UserTypes.CreateUserModel
    ...
    posts: {
      get: (args: { id: string }): Promise<UserTypes.Post> =>
        fetch(
          replacePathAndQueryVariables(
            `${apiBaseUrl}/api/posts/{id}`,
            args,
            undefined
          ),
          {
      ...
      create: (
        args: {} & UserTypes.CreatePostModel
